...
agent.load("./dqn-result/episode17.h5")
...
while episode <= EPISODES:
    ...
    while True:
        ...
        data = bytes.decode(data)
        if data == "over":  # 此次迭代结束
            agent.save("./dqn-result/episode" + str(episode) + ".h5")
            print("weight saved")
            print("episode: {}, epsilon: {}".format(episode, agent.epsilon))
            with open('./dqn-result/train.txt', 'a') as f:
                f.write(" episode {} epsilon {}\n".format(episode, agent.epsilon))
            close_all(sim)
            conn.close()  # 关闭连接
            with open('./dqn-result/exp1.pkl', 'wb') as exp1:
                pickle.dump(agent.memory1, exp1)
            with open('./dqn-result/exp2.pkl', 'wb') as exp2:
